...
<div class="space-y-8">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-3xl font-black text-slate-900 tracking-tight">Outstanding Payments</h1>
            <p class="text-slate-500 font-medium">Unpaid and partially paid bookings with estimated balances</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.finance.revenue.deposits') }}" class="px-5 py-2.5 bg-white border border-slate-200 text-slate-700 font-bold rounded-xl hover:bg-slate-50 transition-all shadow-sm">Deposits</a>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
        <form method="GET" class="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
            ...
            <div class="md:col-span-3 flex items-center gap-3">
                ...
                <a href="{{ route('admin.finance.revenue.outstanding-balances') }}" class="px-5 py-2.5 bg-white border border-slate-200 text-slate-700 font-bold rounded-xl hover:bg-slate-50 transition-all shadow-sm">Reset</a>
            </div>
        </form>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
            ...
            <div class="mt-3 text-3xl font-black text-amber-600">${{ number_format((float) ($stats['outstanding'] ?? 0), 2) }}</div>
        </div>
    </div>
