...
<div class="space-y-8">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-3xl font-black text-slate-900 tracking-tight">Deposits</h1>
            <p class="text-slate-500 font-medium">Deposit expectations based on booking rules and totals</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.finance.revenue.all-bookings') }}" class="px-5 py-2.5 bg-white border border-slate-200 text-slate-700 font-bold rounded-xl hover:bg-slate-50 transition-all shadow-sm">Booking Revenue</a>
            <a href="{{ route('admin.finance.revenue.outstanding-balances') }}" class="px-5 py-2.5 bg-emerald-600 text-white font-bold rounded-xl hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-500/20">Outstanding</a>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
        <form method="GET" class="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
            ...
            <div class="md:col-span-3 flex items-center gap-3">
                ...
                <a href="{{ route('admin.finance.revenue.deposits') }}" class="px-5 py-2.5 bg-white border border-slate-200 text-slate-700 font-bold rounded-xl hover:bg-slate-50 transition-all shadow-sm">Reset</a>
            </div>
        </form>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
            <div class="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Deposit Rule</div>
            <div class="mt-3 text-3xl font-black text-slate-900">{{ number_format((float) ($stats['deposit_percent'] ?? 30), 0) }}%</div>
            <div class="mt-2 text-xs font-bold text-slate-400">From system booking rules (fallback 30%)</div>
        </div>
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
            <div class="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Expected Deposits</div>
            <div class="mt-3 text-3xl font-black text-amber-600">${{ number_format((float) ($stats['expected'] ?? 0), 2) }}</div>
            <div class="mt-2 text-xs font-bold text-slate-400">Estimate across filtered bookings</div>
        </div>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-10 text-center text-slate-400 font-bold">No bookings found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
