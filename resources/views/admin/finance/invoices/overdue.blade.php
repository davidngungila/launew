...
<div class="space-y-8">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-3xl font-black text-slate-900 tracking-tight">Overdue Invoices</h1>
            <p class="text-slate-500 font-medium">Currently defined as unpaid bookings older than {{ $stats['cutoff_days'] ?? 14 }} days</p>
        </div>
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('admin.finance.revenue.outstanding-balances') }}" class="px-5 py-2.5 bg-amber-600 text-white font-bold rounded-xl hover:bg-amber-700 transition-all shadow-lg shadow-amber-500/20">Outstanding</a>
        </div>
    </div>

    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
            <div class="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Overdue Count</div>
            <div class="mt-3 text-3xl font-black text-rose-600">{{ $stats['overdue_count'] ?? 0 }}</div>
        </div>
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
            <div class="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Overdue Amount</div>
            <div class="mt-3 text-3xl font-black text-slate-900">${{ number_format((float) ($stats['overdue_amount'] ?? 0), 2) }}</div>
        </div>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-10 text-center text-slate-400 font-bold">No overdue items found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
