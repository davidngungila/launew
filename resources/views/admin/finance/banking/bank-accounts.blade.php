...
<div class="space-y-8">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-3xl font-black text-slate-900 tracking-tight">Bank Accounts</h1>
            <p class="text-slate-500 font-medium">Track balances and reconciliation status</p>
        </div>
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('admin.finance.banking.accounts.create', ['type' => 'bank']) }}" class="px-5 py-2.5 bg-emerald-600 text-white font-bold rounded-xl hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-500/20">New Account</a>
            <a href="{{ route('admin.finance.banking.bank-accounts.export-pdf') }}" target="_blank" class="px-5 py-2.5 bg-slate-900 text-white font-bold rounded-xl hover:bg-slate-800 transition-all">Export PDF</a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="bg-slate-900 rounded-2xl p-6 text-white shadow-xl md:col-span-2">
            <div class="text-[10px] font-black uppercase tracking-[0.2em] text-white/50">Total Balance (Calculated)</div>
            <div class="mt-3 text-3xl font-black">${{ number_format((float) ($stats['balance_total'] ?? 0), 2) }}</div>
            <div class="mt-2 text-xs font-bold text-white/60">Balance = opening + incoming transfers - outgoing transfers</div>
        </div>
    </div>
            ...
            <table class="w-full text-left">
                <thead>
                    <tr class="bg-slate-50/50">
                        ...
                        <th class="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Institution</th>
                ...
                <tbody class="divide-y divide-slate-50">
                    @forelse($accounts as $a)
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            <td class="px-6 py-5">
                                ...
                                <div class="text-[10px] font-bold text-slate-400">{{ $a->account_number ?? '—' }}</div>
                            </td>
                            <td class="px-6 py-5 text-sm font-black text-slate-900">{{ $a->currency }}</td>
                            <td class="px-6 py-5 text-sm font-bold text-slate-700">{{ $a->institution ?? '—' }}</td>
                            <td class="px-6 py-5">
                    ...
                    @empty
                        <tr><td colspan="6" class="px-6 py-10 text-center text-slate-400 font-bold">No bank accounts found.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
