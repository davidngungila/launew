...
<div class="max-w-4xl mx-auto space-y-8 pb-20">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-3xl font-black text-slate-900 tracking-tight">Edit Recurring Expense</h1>
            <p class="text-slate-500 font-medium">Update planned recurring cost</p>
        </div>
    </div>

    <form action="{{ route('admin.finance.expenses.recurring.update', $recurring) }}" method="POST" class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-10 space-y-8">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="space-y-1.5 md:col-span-2">
                ...
                <input type="text" name="name" value="{{ old('name', $recurring->name) }}" required class="w-full px-5 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-900" />
            </div>
            <div class="space-y-1.5">
                <label class="text-xs font-black text-slate-400 uppercase tracking-widest pl-1">Amount</label>
                <input type="number" step="0.01" name="amount" value="{{ old('amount', $recurring->amount) }}" required class="w-full px-5 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-900" />
            </div>
            <div class="space-y-1.5">
                ...
                <select name="frequency" required class="w-full px-5 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-900">
                    @foreach(['daily','weekly','monthly'] as $f)
                        <option value="{{ $f }}" {{ old('frequency', $recurring->frequency) === $f ? 'selected' : '' }}>{{ strtoupper($f) }}</option>
                    @endforeach
                </select>
            </div>
            <div class="space-y-1.5">
                ...
                <select name="category" required class="w-full px-5 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-900">
                    @foreach($categories as $cat)
                        <option value="{{ $cat->key }}" {{ old('category', $recurring->category) === $cat->key ? 'selected' : '' }}>{{ $cat->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="space-y-1.5">
                <label class="text-xs font-black text-slate-400 uppercase tracking-widest pl-1">Starts On</label>
                <input type="date" name="starts_on" value="{{ old('starts_on', $recurring->starts_on?->format('Y-m-d')) }}" class="w-full px-5 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-900" />
            </div>
            <div class="space-y-1.5">
                <label class="text-xs font-black text-slate-400 uppercase tracking-widest pl-1">Next Run On</label>
                <input type="date" name="next_run_on" value="{{ old('next_run_on', $recurring->next_run_on?->format('Y-m-d')) }}" class="w-full px-5 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-900" />
            </div>
            <div class="flex items-center gap-3 pt-8 md:col-span-2">
                <input id="is_active" type="checkbox" name="is_active" value="1" {{ old('is_active', $recurring->is_active) ? 'checked' : '' }} />
    ...
    <form action="{{ route('admin.finance.expenses.recurring.destroy', $recurring) }}" method="POST" onsubmit="return confirm('Delete this recurring expense?')" class="max-w-4xl mx-auto">
        @csrf
        @method('DELETE')
