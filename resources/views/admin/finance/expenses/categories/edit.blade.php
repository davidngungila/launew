...
<div class="max-w-4xl mx-auto space-y-8 pb-20">
    <div class="flex items-center gap-4">
        ...
        <div>
            ...
            <p class="text-slate-500 font-medium">Update category metadata</p>
        </div>
    </div>

    <form action="{{ route('admin.finance.expenses.categories.update', $category) }}" method="POST" class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-10 space-y-8">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="space-y-1.5">
                ...
                <input type="text" name="key" value="{{ old('key', $category->key) }}" required class="w-full px-5 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-900" />
            </div>
            <div class="space-y-1.5">
                <label class="text-xs font-black text-slate-400 uppercase tracking-widest pl-1">Name</label>
                <input type="text" name="name" value="{{ old('name', $category->name) }}" required class="w-full px-5 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-900" />
            </div>
            <div class="space-y-1.5">
                <label class="text-xs font-black text-slate-400 uppercase tracking-widest pl-1">Sort Order</label>
                <input type="number" name="sort_order" value="{{ old('sort_order', $category->sort_order) }}" class="w-full px-5 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-900" />
            </div>
    ...
    <form action="{{ route('admin.finance.expenses.categories.destroy', $category) }}" method="POST" onsubmit="return confirm('Delete this category?')" class="max-w-4xl mx-auto">
        @csrf
        @method('DELETE')
