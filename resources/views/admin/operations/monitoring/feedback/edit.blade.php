...
<div class="max-w-5xl mx-auto space-y-8 pb-20">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-3xl font-black text-slate-900 tracking-tight">Edit Feedback</h1>
            <p class="text-slate-500 font-medium">Update customer feedback and follow-up status</p>
        </div>
    </div>

    <form action="{{ route('admin.operations.monitoring.feedback.update', $feedback) }}" method="POST" class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-10 space-y-8">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="space-y-1.5">
                ...
                <input type="text" name="customer_name" value="{{ old('customer_name', $feedback->customer_name) }}" class="w-full px-5 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all">
            </div>

            <div class="space-y-1.5">
                <label class="text-xs font-black text-slate-400 uppercase tracking-widest pl-1">Customer Email</label>
                <input type="email" name="customer_email" value="{{ old('customer_email', $feedback->customer_email) }}" class="w-full px-5 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all">
            </div>

            <div class="space-y-1.5">
                <label class="text-xs font-black text-slate-400 uppercase tracking-widest pl-1">Rating (1-5)</label>
                <input type="number" name="rating" min="1" max="5" value="{{ old('rating', $feedback->rating) }}" class="w-full px-5 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all">
            </div>
            ...
            <div class="space-y-1.5">
                ...
                <select name="status" required class="w-full px-5 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-900 focus:outline-none cursor-pointer">
                    @foreach(['new','reviewed','actioned','archived'] as $st)
                        <option value="{{ $st }}" {{ old('status', $feedback->status) === $st ? 'selected' : '' }}>{{ strtoupper($st) }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="space-y-1.5">
                ...
                <input type="datetime-local" name="submitted_at" value="{{ old('submitted_at', $feedback->submitted_at ? $feedback->submitted_at->format('Y-m-d\\TH:i') : '') }}" class="w-full px-5 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all">
            </div>
            ...
            <div class="space-y-1.5">
                ...
                <select name="booking_id" class="w-full px-5 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-900 focus:outline-none cursor-pointer">
                    ...
                    @foreach($bookings as $b)
                        <option value="{{ $b->id }}" {{ (string) old('booking_id', $feedback->booking_id) === (string) $b->id ? 'selected' : '' }}>BK-{{ str_pad($b->id, 4, '0', STR_PAD_LEFT) }} · {{ $b->customer_name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="space-y-1.5 md:col-span-2">
                ...
                <textarea name="message" rows="5" required class="w-full px-5 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all">{{ old('message', $feedback->message) }}</textarea>
            </div>
        </div>
    ...
    <form action="{{ route('admin.operations.monitoring.feedback.destroy', $feedback) }}" method="POST" class="flex justify-end" onsubmit="return confirm('Delete this feedback entry?');">
        @csrf
        @method('DELETE')
