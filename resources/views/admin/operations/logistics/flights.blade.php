...
<div class="space-y-8">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-3xl font-black text-slate-900 tracking-tight">Flight Details</h1>
            <p class="text-slate-500 font-medium">Capture arrivals/departures and coordinate pickups for upcoming clients</p>
        </div>
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('admin.operations.logistics.park-fees') }}" class="px-5 py-2.5 bg-emerald-600 text-white font-bold rounded-xl hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-500/20">Park Fees</a>
        </div>
    </div>

    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
            <div class="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Upcoming (14 Days)</div>
            <div class="mt-3 text-3xl font-black text-slate-900">{{ $stats['upcoming_14_days'] ?? 0 }}</div>
            <div class="mt-1 text-sm font-medium text-slate-500">Prioritize pickups and arrival coordination</div>
        </div>
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
            <div class="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Clients With Phone</div>
            <div class="mt-3 text-3xl font-black text-emerald-600">{{ $stats['with_phone'] ?? 0 }}</div>
            <div class="mt-1 text-sm font-medium text-slate-500">Fast confirmation on arrival time changes</div>
        </div>
    </div>

    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div class="p-6 border-b border-slate-50">
            <div class="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Flight Coordination Queue</div>
            <div class="mt-2 text-sm font-bold text-slate-600">Use customer contact and assignments to coordinate pickups</div>
        </div>
                    ...
                    @forelse($bookings as $booking)
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td class="px-6 py-5">
                                ...
                                <p class="mt-1 text-[10px] font-bold text-slate-400">{{ $booking->customer_phone ?? 'No phone' }}</p>
                            </td>
